...
public class BalloonButton extends JComponent implements GameListener, GameConsts{
	
	public BalloonButton(GameInterface aGame, Coord aCoord){
		this.game = aGame;
		this.coord = aCoord;
		this.label = game.getColorAsString(this.coord);
		this.color = null;
		this.highlight = false;
		this.color = this.convertColor(game.getColor(this.coord));
	}
    
    public void gameEventReceived(GameEvent event){
        if (!event.getCoords().contains(this.coord)) return;
        if (event.getAction() == "update"){
            this.color = this.convertColor(this.game.getColor(this.coord));
        }
...
//		g.drawString(this.label, (int)x, (int)baseY);
    ...
    private Color convertColor(byte aColor){
        Color result;
        switch (aColor){
